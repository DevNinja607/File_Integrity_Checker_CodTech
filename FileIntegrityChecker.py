...
def compute_file_hash(filepath, algo='sha256'):
    ...
    try:
        ...
        print(f"{Fore.RED}Error: {filepath} is a directory, not a file.")
        return None
    ...
    if os.path.exists(file):
        try:
            with open(file, 'r') as hash_file:
                ...
                # Ensure the format is correct for each entry
...
def validate_file_integrity(filepath, algo='sha256', hashes_file='hashes.json'):
    ...
    if os.path.isdir(filepath):
        print(f"{Fore.RED}Error: {filepath} is a directory, not a file. Skipping...")
        return
